...
func display(r [][]rune) {
	for y := 0; y < len(r); y++ {
		for x := 0; x < len(r[y]); x++ {
			if r[y][x] == 'G' {
				termbox.SetCell(x, y, r[y][x], termbox.ColorRed|termbox.AttrBold, termbox.ColorBlack)
			} else if r[y][x] == 'E' {
				termbox.SetCell(x, y, r[y][x], termbox.ColorGreen|termbox.AttrBold, termbox.ColorBlack)
			} else if r[y][x] == '#' {
				termbox.SetCell(x, y, r[y][x], termbox.ColorWhite, termbox.ColorBlack)
			} else {
				termbox.SetCell(x, y, r[y][x], termbox.ColorDefault, termbox.ColorBlack)
			}
		}
	}
	termbox.Flush()
	...
	for y := 0; y < len(playfield); y++ {
		for x := 0; x < len(playfield[y]); x++ {
			if contains(coord{y: y, x: x}, path) {
				termbox.SetCell(x, y, playfield[y][x], termbox.ColorMagenta|termbox.AttrBold|termbox.AttrUnderline, termbox.ColorBlack)
			} else {
				termbox.SetCell(x, y, playfield[y][x], termbox.ColorGreen, termbox.ColorBlack)
			}
		}
	}
	termbox.Flush()
